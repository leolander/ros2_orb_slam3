...
StereoMode::StereoMode() : Node("stereo_node_cpp"), pAgent(nullptr)
{
    ...
    this->declare_parameter("full_settings_path_arg", homeDir + "/" + packagePath + "orb_slam3/config/Stereo/EuRoC.yaml");
    ...
    fullSettingsPath = this->get_parameter("full_settings_path_arg").as_string();
    ...
    RCLCPP_INFO(this->get_logger(), "full_settings_path %s", fullSettingsPath.c_str());

    // Initialize VSLAM directly using parameters
    initializeVSLAM(fullSettingsPath);
...
void StereoMode::initializeVSLAM(std::string& fullSettingsPath){
    if (vocFilePath == "file_not_set" || fullSettingsPath == "file_not_set")
    {
...
void StereoMode::ImgLeft_callback(const sensor_msgs::msg::Image& msg)
{
    ...
    try {
        ...
        if (bRightReceived) {
            ...
            pAgent->TrackStereo(imLeft, imRight, timestamp);
            bLeftReceived = false;
